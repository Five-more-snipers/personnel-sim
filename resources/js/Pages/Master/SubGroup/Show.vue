        ...
        <div class="col-md-8">
          <div class="card shadow-sm border-0">
            <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
              <h4 class="mb-0">{{ subGroup.name }}</h4>
              <div class="d-flex gap-2">
                <Link :href="`/sub-groups/${subGroup.id}/edit`" class="btn btn-sm btn-light">
                  <i class="bi bi-pencil"></i> Edit
                </Link>
                <Link href="/sub-groups" class="btn btn-sm btn-outline-light">
                  ← Back
                </Link>
              </div>
            </div>
            <div class="card-body">
              <div class="mb-3">
                <strong>Faction:</strong> {{ subGroup.faction?.name || '-' }}
              </div>
              <div v-if="subGroup.description" class="biography-content" v-html="renderedDescription">
              </div>
            ...
            <div class="card-body">
              <ul class="list-group list-group-flush">
                <li class="list-group-item d-flex justify-content-between">
                  ...
                  <span class="fw-bold">{{ subGroup.personnels_count || 0 }}</span>
                </li>
              </ul>
            </div>
          </div>
...
const renderedDescription = computed(() => {
  if (!props.subGroup.description) return ''
  return md.render(props.subGroup.description)
})
</script>
