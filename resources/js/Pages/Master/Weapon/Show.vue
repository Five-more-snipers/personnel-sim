<template>
  <AppLayout>
    <div class="container mt-4" v-if="weapon">
      <div class="row">
        <div class="col-md-8">
          <div class="card shadow-sm border-0">
            <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
              <h4 class="mb-0">{{ weapon.name }}</h4>
              <div class="d-flex gap-2">
                <Link :href="`/weapons/${weapon.id}/edit`" class="btn btn-sm btn-light">
                  <i class="bi bi-pencil"></i> Edit
                </Link>
                <Link href="/weapons" class="btn btn-sm btn-outline-light">
                  ← Back
                </Link>
              </div>
            </div>
            ...
            <div class="card-body">
              <ul class="list-group list-group-flush">
                <li class="list-group-item d-flex justify-content-between">
                  ...
                  <span class="fw-bold">{{ weapon.personnels_count || 0 }}</span>
                </li>
              </ul>
            </div>
          </div>
...
const renderedDescription = computed(() => {
  if (!props.weapon?.description) return ''
  return md.render(props.weapon.description)
})
</script>
